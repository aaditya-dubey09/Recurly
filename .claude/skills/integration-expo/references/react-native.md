...
```javascript
posthog.has_opted_out_capturing() // See if a user has opted out
posthog.opt_in_capturing() // opt in
posthog.opt_out_capturing() // opt out
```
...
You can enable debug mode by setting the `debug` prop to `true` on `PostHogProvider`. This will enable verbose logs about the inner workings of the SDK.
