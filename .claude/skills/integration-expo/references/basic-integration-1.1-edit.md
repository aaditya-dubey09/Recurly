...
Where possible, add calls for PostHog's identify() function on the client side upon events like logins and signups. Use only non-sensitive identifiers from login/signup flows (for example: user ID, username, or email) when identifying users on submit. Never send passwords, OTPs, access tokens, or payment data in identify() calls or event properties. If there is server-side code, pass the client-side session and distinct ID to the server-side code to identify the user. On the server side, make sure events have a matching distinct ID where relevant.
